...
def load_sheet_df():
    ...
    df = pd.read_csv(url)
    # 예상 컬럼: 단지명 lat lon 평향 2024년 2025년 신고가 ...
    num_cols = ['평향', '2024년', '2025년', '신고가', 'lat', 'lon']
    for c in num_cols:
        if c in df.columns:
...
def build_dataframe() -> pd.DataFrame:
    ...
    if df[['lat', 'lon']].isna().any().any():
        st.error("❗ 시트에 lat / lon 좌표가 딸리지 않았습니다.")
        st.stop()
...
def build_map(df: pd.DataFrame) -> folium.Map:
    ...
    for name, g in df.groupby('단지명'):
        ...
        folium.Marker([
            lat0, lon0
        ], icon=folium.DivIcon(html=f"<div style='font-size:12px;font-weight:bold;background:rgba(255,255,255,0.75);padding:2px 4px;border-radius:4px;'>{name}</div>")).add_to(m)
        ...
        for i, (_, row) in enumerate(g.iterrows()):
            ...
            folium.CircleMarker([
                ...
                popup=folium.Popup(
                    f"<b>{row['단지명']} {int(row['평향'])}평</b><br>24년 최고가 {money(row['2024년'])}<br>25년 최고가 {money(row['2025년'])}<br>신고가 {shin(row['신고가_유효'])}<br><b>상승률 {rate(row['상승률(%)'])}</b>",
                    max_width=280),
                tooltip=f"{int(row['평향'])}평").add_to(cluster)
            folium.Marker([
                lat_c, lon_c
            ], icon=folium.DivIcon(html=f"<div style='font-size:11px;font-weight:bold;transform:translate(-50%,-12px);'>{int(row['평향'])}평</div>")).add_to(m)
    ...
    <div class='overlay-box report-btn'>
        <a href='{FORM_URL}' target='_blank' style='background:#007bff; color:#fff; padding:10px 18px; border-radius:6px;
